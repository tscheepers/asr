...
extension Array where Element : FloatingPoint {
    ...
    func shortTimeFourierTransform(nFFT: Int = 2048, hopLength: Int? = nil, window: STFTWindowFunction = .hann, reflect: Bool = true) -> ComplexMatrix<Element> {
        ...
        switch window {
        case .hann:
            w = [Element].hannWindow(windowLength: nFFT)
        case .hamming:
            w = [Element].hammingWindow(windowLength: nFFT)
        }
        let tamperedFrames = frames * w.broadcast(size: frames.height)
    ...
    /// Create an array with values from the Hanming function
    /// Read: https://en.wikipedia.org/wiki/Window_function
    static func hammingWindow(windowLength: Int) -> [Element] {
        // On an even window length we make sure there is always a peak of 1.0 by adding an extra item
        let evenCorrection = (windowLength % 2 == 0 ? 1 : 0)
        return interpolation(start: 0, stop: 2*Element.pi, count: windowLength + evenCorrection)
            .map { 27 / 50 - 23 * cos($0) / 50 }.dropLast(evenCorrection)
